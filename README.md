...
This is a port of the original LinqToExcel to .NET 6
<hr>
...
## Using the LinqToExcelModern.Row class

Query results can be returned as LinqToExcelModern.Row objects which allows you to access a cell's value by using the column name in the string index. Just use the `Worksheet()` method without a generic argument.
...
The LinqToExcelModern.Row class allows you to easily cast a cell's value by using its `Cast<>()` method
